...
import type { LayoutServerLoad } from "../../routes/public/$types";
// import type { LayoutServerLoad } from 
export const load: LayoutServerLoad = async (ld) => {
    const parser = new UrlParser(ld.url.href);
    let colourMode: colourMode = validateColourmode(ld.cookies.get('colourMode'));
    if (!ld.cookies.get('colourMode')) ld.cookies.set('colourMode', 'dark_default', { path: '/' });
    return {
...
function toMetaData(parser: UrlParser, url: URL) {
    const type = url.pathname.includes('.') ? 'file' : 'folder';
    const path = url.pathname.split('/');
    if (type == 'file') {
        const fname = `File preview - ${path.slice().pop() ?? 'unknown'}`;
